...
value "algunos (\<lambda>x. 1<length x) [[2,1,4],[5]]"
...
have "todos (\<lambda>x. P x \<and> Q x) (a # xs) = (P a \<and> Q a \<and> todos (\<lambda>x. P x \<and> Q x) xs)" by simp
also have "... = (P a \<and> Q a \<and> todos P xs \<and> todos Q xs)" using HI by simp
also have "... = (P a \<and> todos P xs \<and> Q a \<and> todos Q xs)"  by arith
...
lemma todos_append:
  "todos P (x @ y) = (todos P x \<and> todos P y)"
proof (induct x)
show " todos P ([] @ y) = (todos P [] \<and> todos P y)" using [[simp_trace]]  by simp
next
fix a x 
...
also have "... = (todos P xs \<and> todos P [a])" using HI by simp
also have "... = (todos P [a] \<and> todos P xs)" by (simp add: HOL.conj_commute)
also have "... = todos P([a]@(xs))" by (simp)
finally show  "todos P (rev (a#xs))= todos P (a # xs)" by simp
qed


value "rev[1,5] = (rev [5] @ [1])"
text {*
  --------------------------------------------------------------------- 
