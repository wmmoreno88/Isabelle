...
text {*
  ...
  · "proof cases" indica que el método de demostración será por
    distinción de casos. 
  · Se generan 2 casos:
       1. ?P \<Longrightarrow> \<not>A \<or> A
       2. \<not>?P \<Longrightarrow> \<not>A \<or> A
...
text {*
  ...
  · "case False" indica que se está suponiendo que A es falsa. Es
    equivalente a "assume \<not>A".
  · En general, 
    · el método (cases F) es una abreviatura de la aplicación de la regla
         \<lbrakk>F \<Longrightarrow> Q; \<not>F \<Longrightarrow> Q\<rbrakk> \<Longrightarrow> Q  
    · La expresión "case True" es una abreviatura de F.
    · La expresión "case False" es una abreviatura de \<not>F.
...
text {*
  ...
  · Se generan 2 casos:
       1. xs = [] \<Longrightarrow> length (tl xs) = length xs - 1
       2. \<And>a list. xs = a # list \<Longrightarrow> length (tl xs) = length xs - 1
*}
...
text {*
  ...
  entonces n+1 también la tiene. 
     \<lbrakk>P 0; \<And>n. P n \<Longrightarrow> P (Suc n)\<rbrakk> \<Longrightarrow> P m
...
text {*
  ...
  · El esquema de inducción estructural sobre listas es
    · list.induct: \<lbrakk>P []; \<And>x ys. P ys \<Longrightarrow> P (x # ys)\<rbrakk> \<Longrightarrow> P zs
...
fun espejo :: "'a arbolB \<Rightarrow> 'a arbolB" where
...
next
  ...
  proof -
    ...
    also have "\<dots> = Nodo x (espejo (espejo i)) (espejo (espejo d))" 
      by simp
    also have "\<dots> = Nodo x i d" using h1 h2 by simp 
    finally show ?thesis .
 qed
qed
...
text {*
  ...
  · Se generan dos casos:
    1. \<And>a. espejo (espejo (Hoja a)) = Hoja a
    2. \<And>a1 a2 a3. \<lbrakk>espejo (espejo a2) = a2; 
                   espejo (espejo a3) = a3\<rbrakk>
                  \<Longrightarrow> espejo (espejo (Nodo a1 a2 a3)) = Nodo a1 a2 a3
*}
...
fun aplana :: "'a arbolB \<Rightarrow> 'a list" where
  "aplana (Hoja x)     = [x]"
...
next
  ...
  proof -
    ...
    also have "\<dots> = (aplana(espejo d))@[x]@(aplana(espejo i))" by simp
...
definition inversaAc :: "'a list \<Rightarrow> 'a list" where
  "inversaAc xs \<equiv> inversaAcAux xs []"
...
next
  ...
  have "inversaAc (a#xs) = inversaAcAux (a#xs) []" 
    by (simp add: inversaAc_def)
  also have "\<dots> = inversaAcAux xs [a]" by simp
  also have "\<dots> = inversa (a#xs)"
...
proof (induct xs arbitrary: ys)
  show "\<And>ys. inversaAcAux [] ys = (inversa [])@ys" by simp
next
  fix a xs 
  assume HI: "\<And>ys. inversaAcAux xs ys = inversa xs@ys"
  show "\<And>ys. inversaAcAux (a#xs) ys = inversa (a#xs)@ys"
  proof -
    fix ys
    have "inversaAcAux (a#xs) ys = inversaAcAux xs (a#ys)" by simp
    also have "\<dots> = inversa xs@(a#ys)" using HI by simp
    also have "\<dots> = inversa (a#xs)@ys" using [[simp_trace]] by simp 
...
proof -
  ...
  also have "\<dots> = (inversa xs)@([a]@ys)" by (simp only: append.simps(2))
  also have "\<dots> = ((inversa xs)@[a])@ys" by (simp only: append_assoc)
  also have "\<dots> = (inversa (a#xs))@ys" by (simp only: inversa.simps(2))
  finally show ?thesis .
qed
...
text {*
  ...
  · Al definir una función recursiva general se genera una regla de
    ...
    ack.induct: 
       \<lbrakk>\<And>n. P 0 n; 
        \<And>m. P m 1 \<Longrightarrow> P (Suc m) 0;
        \<And>m n. \<lbrakk>P (Suc m) n; P m (ack (Suc m) n)\<rbrakk> \<Longrightarrow> P (Suc m) (Suc n)\<rbrakk>
...
text {*
  ...
  · Se generan 3 casos:
    1. \<And>n. n < ack 0 n
    2. \<And>m. 1 < ack m 1 \<Longrightarrow> 0 < ack (Suc m) 0
    3. \<And>m n. \<lbrakk>n < ack (Suc m) n; 
              ack (Suc m) n < ack m (ack (Suc m) n)\<rbrakk>
             \<Longrightarrow> Suc n < ack (Suc m) (Suc n)
*}
...
text {*
  ...
  La regla de inducción sobre árboles y bosques es arbol_bosque.induct:
     \<lbrakk>P1 Hoja; 
      \<And>x b. P2 b \<Longrightarrow> P1 (Nodo x b); 
      P2 Vacio;
      \<And>a b. \<lbrakk>P1 a; P2 b\<rbrakk> \<Longrightarrow> P2 (ConsB a b)\<rbrakk> 
     \<Longrightarrow> P1 a \<and> P2 b
*}
...
lemma "aplana_arbol  (map_arbol  f a) = map f (aplana_arbol a)
     \<and> aplana_bosque (map_bosque f b) = map f (aplana_bosque b)"
...
next
  ...
  also have "\<dots> = (f x)#(aplana_bosque (map_bosque f b))" by simp
  also have "\<dots> = (f x)#(map f (aplana_bosque b))" using HI by simp
  also have "\<dots> = map f (aplana_arbol (Nodo x b))" by simp
...
next
  ...
  also have "\<dots> = aplana_arbol(map_arbol f a)@aplana_bosque(map_bosque f b)" 
    by simp
  also have "\<dots> = (map f (aplana_arbol a))@(map f (aplana_bosque b))" 
    using HI1 HI2 by simp
  also have "\<dots> = map f (aplana_bosque (ConsB a b))" by simp
...
text {*
  ...
  · Se generan 4 casos:
    ...
    2. \<And>a bosque.
          aplana_bosque (map_bosque bosque h) = map h (aplana_bosque bosque) \<Longrightarrow>
    ...
    4. \<And>arbol bosque.
          \<lbrakk>aplana_arbol (map_arbol arbol h) = map h (aplana_arbol arbol);
           aplana_bosque (map_bosque bosque h) = map h (aplana_bosque bosque)\<rbrakk>
          \<Longrightarrow> aplana_bosque (map_bosque (ConsB arbol bosque) h) =
